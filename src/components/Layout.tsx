...
export default function Layout() {
  ...
  const secondaryNav = [
    ...
    ...(user.role === "admin"
      ...
      : []),
    ...(user.email === "22247071347@bosla.app"
      ? [{ name: "إدارة النظام", href: "/admin", icon: Settings }]
      : []),
  ];

  let navigation = [...primaryNav, ...secondaryNav];

  if (activeStaff && activeStaff.role !== "manager") {
    if (activeStaff.role === "staff") {
      navigation = navigation.filter(nav => nav.href === "/");
    } else if (activeStaff.role === "staff_2") {
      navigation = navigation.filter(nav => ["/", "/bookings", "/customers", "/suppliers", "/transactions", "/expenses"].includes(nav.href));
    }
  }

  return (
    <div
      className="min-h-screen bg-slate-50 flex text-slate-800"
    ...
    >
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden w-full relative">
        ...
        {isExpired && (
          <div className="bg-amber-50 border-b border-amber-200 py-2.5 px-4 md:px-8 text-center text-amber-800 text-sm font-medium z-10">
            عذراً، لقد انتهت الفترة التجريبية. للتجديد ومواصلة العمل، يرجى تفعيل
            الاشتراك من الإعدادات أو التواصل عبر واتساب على{" "}
            <a
              href="https://wa.me/22247071310"
              target="_blank"
        ...
        <header className="h-16 md:h-[72px] bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-8 shrink-0">
          ...
          <div
            className={cn(
              "hidden md:block relative flex-1 max-w-md",
              language === "ar" ? "ml-4" : "mr-4",
            )}
          >
            <Search
              className={cn(
                "w-4 h-4 absolute top-1/2 -translate-y-1/2 text-slate-400",
          ...
          <div className="flex items-center gap-4">
            {staffMembers.length > 0 && (
              <div className="flex items-center gap-2">
                ...
                >
                  <option value="">
                    {user.name} ({language === "ar" ? "أساسي" : "Principal"})
                  </option>
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8 pb-24 md:pb-8">
          {navigation.some(nav => nav.href === location.pathname) || (location.pathname === "/ai-analyst" && (!activeStaff || activeStaff.role === "manager")) ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <h2 className="text-xl font-bold text-slate-700 mb-2">ليس لديك صلاحية للوصول</h2>
              <p className="text-slate-500">الرجاء التواصل مع الإدارة لطلب الصلاحيات اللازمة.</p>
              <Link to="/" className="mt-4 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700">
                العودة للرئيسية
              </Link>
            </div>
          )}
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 flex justify-around items-center h-16 z-40 pb-safe">
        {navigation.filter(nav => primaryNav.some(p => p.href === nav.href)).map((item) => {
          const isActive = location.pathname === item.href;
          return (
            <Link
              key={item.name}
        ...
        >
          <Menu
            className={cn(
              "w-5 h-5",
              isMobileMenuOpen ? "opacity-100" : "opacity-70",
            )}
          />
        ...
        <div className="fixed inset-0 bg-slate-800/50 flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-xl p-6 w-full max-w-xs shadow-xl text-center">
            ...
            <input
              ...
              className={cn(
                "w-full text-center tracking-[1em] font-bold text-xl px-4 py-3 border border-slate-200 rounded-lg focus:ring-2 focus:outline-none mb-4",
                pinError
            ...
            {pinError && (
              <p className="text-xs text-red-500 mb-4">
                {language === "ar" ? "الرمز غير صحيح" : "Code PIN incorrect"}
              </p>
            )}
          ...
          <div className="bg-white rounded-2xl w-full max-w-md p-6 sm:p-8 shadow-xl relative animate-in zoom-in-95 duration-200">
            ...
            <div className="text-center">
              ...
              <p className="text-slate-600 mb-6 text-[15px] leading-relaxed">
                لقد انتهت فترة الـ 7 أيام التجريبية المجانية. الآن إمكانيات
                التطبيق مقيدة ولا يمكنك إضافة بيانات جديدة. نرجو تفعيل الاشتراك
                لمواصلة العمل.
              </p>

              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 mb-6">
                <p className="text-sm text-slate-700 font-medium mb-2">
                  للحصول على قسيمة تفعيل، يرجى التواصل عبر يواتساب:
                </p>
                <a
                  href="https://wa.me/22247071310"
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center justify-center gap-2 text-emerald-600 font-bold bg-emerald-50 px-4 py-2 rounded-lg hover:bg-emerald-100 transition-colors"
                >
                  22247071310
                </a>
              </div>
