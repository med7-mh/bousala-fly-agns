...
export default function Reports() {
  ...
  const bookingsByType = filteredBookings.reduce(
    (acc, b) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 no-print">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            ملخص النشاط المالي والتشغيلي للوكالة
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div
          className={`rounded-xl border p-5 flex items-center justify-between ${netCashFlow >= 0 ? "bg-blue-50 border-blue-100" : "bg-orange-50 border-orange-100"}`}
        >
          <div
            className={`w-12 h-12 rounded-full flex items-center justify-center ${netCashFlow >= 0 ? "bg-blue-100 text-blue-600" : "bg-orange-100 text-orange-600"}`}
          >
            <Receipt className="w-6 h-6" />
          </div>
          <div className="text-left" dir="ltr">
            <div
              className={`text-[13px] font-semibold text-right ${netCashFlow >= 0 ? "text-blue-800" : "text-orange-800"}`}
            >
              صافي حركة الخزينة
            </div>
            <p
              className={`text-2xl font-bold text-right ${netCashFlow >= 0 ? "text-blue-600" : "text-orange-600"}`}
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-800 rounded-xl border border-slate-900 p-5 shadow-sm text-white">
          <div className="text-sm font-semibold text-slate-300 mb-1">
            أرباح المبيعات التقريبية
          </div>
          <p className="text-2xl font-bold text-emerald-400 mt-2">
            {formatCurrency(expectedProfit, t("currency", language))}
          </p>
        </div>
      </div>
        ...
        <div className="p-5">
          {Object.keys(bookingsByType).length === 0 ? (
            <div className="text-center py-6 text-slate-500">
              لا توجد خدمات مسجلة في هذه الفترة
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {Object.entries(bookingsByType).map(([type, count]) => {
                const typeCost = filteredBookings
                  .filter((b) => b.type === type)
                  .reduce((s, b) => s + (b.cost_price || 0), 0);
                const typeRev = filteredBookings
                  .filter((b) => b.type === type)
                  .reduce((s, b) => s + (b.selling_price || 0), 0);
                const typeProfit = typeRev - typeCost;

                return (
                  <div
                    key={type}
                    className="flex flex-col p-4 border border-slate-100 rounded-xl bg-slate-50/50"
                  >
                    ...
                    <div className="flex justify-between items-center text-sm mb-1">
                      ...
                      <span className="font-semibold text-slate-800">
                        {formatCurrency(typeRev, t("currency", language))}
                      </span>
                    </div>
                    <div className="flex justify-between items-center text-sm pt-2 mt-1 border-t border-slate-100">
                      ...
                      <span className="font-bold text-emerald-600">
                        {formatCurrency(typeProfit, t("currency", language))}
                      </span>
                    </div>
                  </div>
                );
