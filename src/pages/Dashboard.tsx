    ...
    <div className="flex flex-col gap-6" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      ...
      {selectedDate === new Date().toISOString().split('T')[0] && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <button 
            onClick={() => openQuickAction(t('form', language), 'income', 100, t('form', language))}
          ...
          >
            ...
            <span className="font-semibold text-slate-800 text-sm">{t('form', language)}</span>
          </button>

          <button 
            onClick={() => openQuickAction(t('photocopy', language), 'income', 50, t('photocopy', language))}
            className="bg-white border hover:border-emerald-500 border-slate-200 rounded-xl p-4 flex flex-col items-center justify-center gap-2 transition-all hover:shadow-md group"
          >
            ...
            <span className="font-semibold text-slate-800 text-sm">{t('photocopy', language)}</span>
          </button>
          ...
          >
            ...
            <span className="font-semibold text-slate-800 text-sm">{t('visa', language)}</span>
          </button>

          <button 
            onClick={() => openQuickAction(t('expense_quick', language), 'operating_expense', 0, t('expense_quick', language))}
          ...
          >
            ...
            <span className="font-semibold text-slate-800 text-sm">{t('expense_quick', language)}</span>
          </button>
        </div>
      )}
        ...
        <div className="overflow-x-auto p-4">
          {dailyTransactions.length > 0 ? (
            <div className="space-y-3">
              {dailyTransactions.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(tObj => (
                <div key={tObj.id} className="flex justify-between items-center p-3 rounded-lg border border-slate-100 bg-white shadow-sm">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${tObj.type === 'income' ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}>
