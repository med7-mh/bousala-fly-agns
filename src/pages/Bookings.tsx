...
import { Plus, Search, Filter, Edit2, Trash2, AlertTriangle, ScanLine, X } from 'lucide-react';
...
export default function Bookings() {
  ...
  const [formServices, setFormServices] = useState<ServiceItem[]>([initialService()]);
  ...
  const getMultiServices = (description: string): ServiceItem[] | null => {
    ...
    try {
      if (text.trim().startsWith('{')) {
        ...
        if (parsed && typeof parsed === 'object' && parsed.is_multi && Array.isArray(parsed.services)) {
  ...
  useEffect(() => {
    if (editingBooking) {
      ...
      if (parsed) {
        setFormServices(parsed.map((s, index) => ({
          ...s,
        ...
        setFormServices([
          {
            ...
            description: parseDescriptionWithStaff(editingBooking.description).text,
  ...
  // Sync total costs
  useEffect(() => {
    const totalCost = formServices.reduce((sum, s) => sum + (Number(s.cost_price) || 0), 0);
    const totalSelling = formServices.reduce((sum, s) => sum + (Number(s.selling_price) || 0), 0);
    setCostPrice(totalCost);
    setSellingPrice(totalSelling);
  }, [formServices]);

  const updateServiceItem = (itemId: string, updates: Partial<ServiceItem>) => {
    setFormServices(prev => prev.map(item => item.id === itemId ? { ...item, ...updates } : item));
  };
  ...
  const filteredBookings = visibleBookings.filter(b => {
    ...
    if (parsedServices) {
      ...
      return text.toLowerCase().includes(searchLower) || (b.type || '').toLowerCase().includes(searchLower);
    }
  });
  ...
  const handleAddBooking = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (selectedCustomerId === 'new') {
      ...
      if (existingCustomer) {
        ...
        const firstNationalId = formServices[0]?.national_id || '';
        if (firstNationalId && existingCustomer.national_id !== firstNationalId) {
    ...
    if (formServices.length === 1) {
      ...
      const isStandard = ['flight', 'hotel', 'visa', 'tour', 'passport'].includes(s.type);
      if (isStandard) {
        finalType = s.type;
        const parts = [];
  ...
  return (
    <div className="flex flex-col gap-6" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <section className="bg-white rounded-xl border border-slate-200 p-5 flex flex-col shadow-sm">
          ...
          <table className="w-full text-right border-collapse">
            <thead>
              <tr className="border-[0px]">
                ...
                <th className="py-3 px-2 border-b-2 border-slate-100 text-slate-400 text-[13px] font-semibold whitespace-nowrap">الخدمات المشمولة</th>
                <th className="py-3 px-2 border-b-2 border-slate-100 text-slate-400 text-[13px] font-semibold whitespace-nowrap">الوصف والتفاصيل</th>
            ...
            <tbody>
              {filteredBookings.map(booking => {
                ...
                const { staffName } = parseDescriptionWithStaff(booking.description);
                
                return (
                  <tr key={booking.id} className="hover:bg-slate-50/85 transition-colors">
                    <td className="py-3.5 px-2 border-b border-slate-100/50 text-[13px] text-slate-500 whitespace-nowrap font-medium" dir="ltr">
                      ...
                      {parsedMulti ? (
                        <div className="flex flex-wrap gap-1">
                          {parsedMulti.map((srv, idx) => (
                            <span key={idx} className="inline-flex items-center px-2 py-0.5 rounded-full bg-slate-100 text-slate-700 text-[11px] font-semibold border border-slate-200">
                    ...
                    <td className="py-3.5 px-2 border-b border-slate-100/50 text-[13px] text-slate-600 min-w-[200px]">
                      {parsedMulti ? (
                        <div className="flex flex-col gap-1.5 py-1">
                          {parsedMulti.map((srv, idx) => (
                            <div key={idx} className="text-[12px] text-slate-600 border-r-2 border-emerald-400 pr-2 py-0.5 bg-slate-50/50 p-1.5 rounded-md">
                              <span className="font-bold text-slate-700">({typeLabels[srv.type] || srv.type}):</span>{' '}
                        ...
                        (() => {
                          const { text } = parseDescriptionWithStaff(booking.description);
                          return (
                    ...
                    <td className="py-3.5 px-2 border-b border-slate-100/50 text-[14px] text-slate-600 whitespace-nowrap font-medium">{formatCurrency(booking.cost_price)}</td>
                    <td className="py-3.5 px-2 border-b border-slate-100/50 text-[14px] text-slate-800 font-bold whitespace-nowrap">{formatCurrency(booking.selling_price)}</td>
                    <td className="py-3.5 px-2 border-b border-slate-100/50 text-[14px] text-emerald-600 font-bold whitespace-nowrap">{formatCurrency(profit)}</td>
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-2xl border border-slate-200 shadow-2xl max-h-[90vh] flex flex-col transition-all">
            <div className="flex justify-between items-center mb-4 border-b border-slate-100 pb-3">
              <h3 className="text-[17px] font-bold text-slate-800">
                {editingBooking ? 'تعديل الحجز وتفاصيل الخدمات' : 'إضافة حجز جديد (متعدد الخدمات)'}
              </h3>
              <button 
                type="button" 
                onClick={() => {
            ...
            <form onSubmit={handleAddBooking} className="space-y-5 overflow-y-auto flex-1 pr-1 pl-1">
              {/* Select Customer */}
              <div className="bg-slate-50/50 p-4 rounded-xl border border-slate-120 space-y-3">
                <div>
                  <label className="block text-[13px] font-bold text-slate-700 mb-1">{t('customer', language)}</label>
                  <select 
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center bg-slate-100/50 px-3 py-2 rounded-lg">
                  <span className="text-xs font-bold text-slate-600">جدول الخدمات المسجلة في هذا الحجز</span>
                  <button
                    type="button"
                ...
                {formServices.map((service, index) => (
                  <div key={service.id} className="relative bg-slate-50 border border-slate-200 p-4 rounded-xl space-y-4 hover:border-slate-300 transition-colors">
                    <div className="flex justify-between items-center pb-2 border-b border-slate-200/50">
                      <span className="text-[13px] font-bold text-slate-700">الخدمة #{index + 1}</span>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-[12px] font-semibold text-slate-600 mb-1">نوع الخدمة المشمولة</label>
                        <div className="flex gap-1.5">
                          <select
                            value={service.type}
                            onChange={(e) => updateServiceItem(service.id, { type: e.target.value })}
                          ...
                          <button
                            type="button"
                            onClick={() => {
                              const name = prompt('أدخل اسم الخدمة المخصصة الجديدة (مثال: تأمين صحي، شحن بري):');
                              if (name && name.trim()) {
                                addCustomBookingType(name.trim());
                                updateServiceItem(service.id, { type: name.trim() });
                              }
                            }}
                      ...
                      <div>
                        <label className="block text-[12px] font-semibold text-slate-600 mb-1">المورد (Suppliers)</label>
                        <select
                          value={service.supplier_id || ''}
                          onChange={(e) => updateServiceItem(service.id, { supplier_id: e.target.value })}
                    ...
                    {(service.type === 'passport' || service.type === 'visa') ? (
                      <>
                        <div className="grid grid-cols-2 gap-3 animate-fadeIn">
                          <div>
                            <label className="block text-[12px] font-semibold text-slate-600 mb-1">الرقم الوطني</label>
                            <input
                              value={service.national_id || ''}
                              onChange={(e) => updateServiceItem(service.id, { national_id: e.target.value })}
                              type="text"
                          ...
                          {service.type === 'visa' ? (
                            <div>
                              <label className="block text-[12px] font-semibold text-slate-600 mb-1">رقم جواز السفر</label>
                              <input
                                value={service.passport_number || ''}
                                onChange={(e) => updateServiceItem(service.id, { passport_number: e.target.value })}
                                type="text"
                            ...
                            <div>
                              <label className="block text-[12px] font-semibold text-slate-600 mb-1">رقم الإيصال الإداري</label>
                              <input
                                value={service.receipt_number || ''}
                                onChange={(e) => updateServiceItem(service.id, { receipt_number: e.target.value })}
                                type="text"
                        ...
                        <div className="grid grid-cols-2 gap-3 animate-fadeIn">
                          {service.type === 'visa' && (
                            <div>
                              <label className="block text-[12px] font-semibold text-slate-600 mb-1 font-medium">رقم الإيصال</label>
                              <input
                                value={service.receipt_number || ''}
                                onChange={(e) => updateServiceItem(service.id, { receipt_number: e.target.value })}
                                type="text"
                          ...
                          <div className={service.type !== 'visa' ? 'col-span-2' : ''}>
                            <label className="block text-[12px] font-semibold text-slate-600 mb-1">تاريخ الاستلام المتوقع والجاهزية</label>
                            <input
                              value={service.expected_date || ''}
                              onChange={(e) => updateServiceItem(service.id, { expected_date: e.target.value })}
                              type="date"
                              className="w-full px-2.5 py-1.5 border border-slate-200 rounded-lg text-xs focus:ring-2 focus:ring-emerald-500"
                            />
                          </div>
                        ...
                        <div>
                          <label className="block text-[12px] font-semibold text-slate-600 mb-1">ملاحظات إضافية على الخدمة</label>
                          <input
                            required
                      ...
                      <>
                        <div className="grid grid-cols-3 gap-3 animate-fadeIn">
                          <div className="col-span-2">
                            <label className="block text-[12px] font-semibold text-slate-600 mb-1">بيانات الخدمة والوصف الكامل</label>
                            <input
                              required
                              value={service.description || ''}
                              onChange={(e) => updateServiceItem(service.id, { description: e.target.value })}
                          ...
                          <div>
                            <label className="block text-[12px] font-semibold text-slate-600 mb-1">رمز الحجز (PNR / Ref)</label>
                            <input
                              value={service.pnr || ''}
                              onChange={(e) => updateServiceItem(service.id, { pnr: e.target.value })}
                              type="text"
                              placeholder="اختياري"
                    ...
                    <div className="grid grid-cols-2 gap-3 pt-1 border-t border-slate-200/30">
                      <div>
                        <label className="block text-[11px] font-semibold text-slate-500 mb-0.5">تكلفة الخدمة (Cost)</label>
                        <input
                          required
                          value={service.cost_price || ''}
                          onChange={(e) => updateServiceItem(service.id, { cost_price: Number(e.target.value) })}
                          type="number"
                          min="0"
                          placeholder="0"
                      ...
                      <div>
                        <label className="block text-[11px] font-semibold text-slate-500 mb-0.5">سعر البيع للعميل (Selling)</label>
                        <input
                          required
                          value={service.selling_price || ''}
                          onChange={(e) => updateServiceItem(service.id, { selling_price: Number(e.target.value) })}
                          type="number"
                          min="0"
                          placeholder="0"
                          className="w-full px-2.5 py-1 border border-slate-200 rounded-lg text-xs font-semibold focus:ring-2 focus:ring-emerald-500"
              ...
              {(sellingPrice > 0 || costPrice > 0) && (
                <div className={`p-4 rounded-xl text-sm flex justify-between font-bold border ${sellingPrice - costPrice >= 0 ? 'bg-emerald-50 border-emerald-100 text-emerald-800' : 'bg-red-50 border-red-100 text-red-800'}`}>
                  <span>إجمالي الحجز المجمع:</span>
                  <div className="flex gap-4">
                    <span>التكلفة: {formatCurrency(costPrice)}</span>
                    <span>البيع: {formatCurrency(sellingPrice)}</span>
                    <span>الربح الإجمالي: {formatCurrency(sellingPrice - costPrice)}</span>
                  </div>
                </div>
              )}
              ...
              {!editingBooking && (
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3">
                  <div>
                    <label className="block text-[13px] font-bold text-slate-700 mb-1">{t('advance_payment', language)}</label>
                    <input 
                      type="number" 
                      min="0" 
                      step="1" 
                  ...
                  <p className="text-[11px] text-slate-500">سيتم تلقائياً إصدار سند قبض مالي بهذا المبلغ وسيكون الحجز مدفوعاً جزئياً.</p>
                </div>
              )}

              {/* General Booking status */}
              <div>
                <label className="block text-[13px] font-bold text-slate-700 mb-1">الحالة الإجمالية للحجز</label>
                <select defaultValue={editingBooking?.status} required name="status" className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:outline-none text-sm bg-white cursor-pointer">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
                ...
                <button type="submit" className="px-5 py-2 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-semibold rounded-lg transition-colors shadow-md shadow-emerald-250">
                  {editingBooking ? 'حفظ الحجز المعدل' : 'تأكيد وحفظ الحجز المجمع'}
                </button>
              </div>
            </form>
          </div>
