...
export default function Transactions() {
  ...
  const filteredTransactions = visibleTransactions.filter((t) => {
    ...
    let match = true;
    if (searchTerm) {
      if (!t.description.toLowerCase().includes(searchTerm.toLowerCase()))
        match = false;
    }
    if (filterType !== "all") {
      ...
      if (
        ...
        match = false;
      if (filterType === "form" && !t.description.includes("استمارة"))
        match = false;
      if (filterType === "photography" && !t.description.includes("تصوير"))
        match = false;
      if (filterType === "visa" && !t.description.includes("تأشير"))
        match = false;
    }
  ...
  const basePaymentMethodsList = [
    ...
    {
      id: "bankily",
      label: language === "ar" ? "بنكيلي" : "Bankily",
      icon: Smartphone,
      color: "text-blue-500",
      bg: "bg-blue-50",
    },
    {
      id: "masrivi",
      label: language === "ar" ? "مصرفي" : "Masrivi",
      icon: Landmark,
  ...
  const getBalanceByMethod = (method: string) => {
    const methodTxs = visibleTransactions.filter(
      (t) =>
        t.payment_method === method || (!t.payment_method && method === "cash"),
    );
    const income = methodTxs
      .filter((t) => t.type === "income")
      .reduce((sum, t) => sum + t.amount, 0);
    const expense = methodTxs
      .filter((t) => t.type === "expense" || t.type === "operating_expense")
      .reduce((sum, t) => sum + t.amount, 0);
  ...
  const handleOpenEditModal = (t: Transaction) => {
    ...
    setSelectedBooking(t.booking_id || "");
    setSelectedCustomer(t.customer_id || "");
    setSelectedSupplier(t.supplier_id || "");
    setIsModalOpen(true);
  };
  ...
  const handleTransferSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const fromLabel =
      paymentMethodsList.find((m) => m.id === fromAccount)?.label ||
      fromAccount;
    const toLabel =
      paymentMethodsList.find((m) => m.id === toAccount)?.label || toAccount;

    const baseDesc =
      description || `تحويل داخلي من ${fromLabel} إلى ${toLabel}`;
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5 flex flex-col gap-2">
          <div className="flex items-center justify-between">
            ...
            <div
              className={`w-8 h-8 rounded-full flex items-center justify-center ${balance >= 0 ? "bg-emerald-50" : "bg-red-50"}`}
            >
              <span
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center ${method.bg}`}
                >
              ...
              <div className="mt-2">
                <p
                  className={`text-lg font-bold ${methodBalance >= 0 ? "text-slate-800" : "text-red-600"}`}
                >
          ...
          <table className="w-full text-right border-collapse">
            <thead>
              <tr>
                <th
                  className={cn(
                    "py-3 px-2 border-b-2 border-slate-100 text-slate-400 text-[13px] font-semibold whitespace-nowrap",
            ...
            <tbody>
              {filteredTransactions.map((transaction) => {
                const booking = transaction.booking_id
                  ? bookings.find((b) => b.id === transaction.booking_id)
                  : null;
                ...
                return (
                  ...
                  >
                    <td
                      className="py-3.5 px-2 border-b border-slate-50 text-[14px] text-slate-500 whitespace-nowrap"
                      dir="ltr"
                    >
                    ...
                    <td
                      className={cn(
                        "py-3.5 px-2 border-b border-slate-50 text-[14px] whitespace-nowrap",
                        language === "ar" ? "text-right" : "text-left",
                      )}
                    >
                      <span
                      ...
                      >
                        ...
                        {transaction.type === "income"
                          ? t("transaction_income", language)
                          : transaction.type === "operating_expense"
                            ? t("operating_expense_short", language)
                            : t("transaction_expense", language)}
                      </span>
                    </td>
                    <td
                      className={cn(
                        "py-3.5 px-2 border-b border-slate-50 text-[14px] font-bold whitespace-nowrap",
                        language === "ar" ? "text-right" : "text-left",
                        transaction.type === "income"
                          ? "text-emerald-600"
                          : "text-red-600",
                    ...
                    <td
                      className={cn(
                        "py-3.5 px-2 border-b border-slate-50 text-[14px] text-slate-700 whitespace-nowrap",
                        language === "ar" ? "text-right" : "text-left",
                      )}
                    >
                      {t(
                        (transaction.payment_method as any) || "cash",
                        language,
                      )}
                    </td>
                    <td
                      className={cn(
                        "py-3.5 px-2 border-b border-slate-50 text-[14px] text-slate-800 min-w-[150px]",
                        language === "ar" ? "text-right" : "text-left",
                      )}
                    >
                      {(() => {
                      ...
                      <div className="flex items-center justify-between gap-4">
                        {booking ? (
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="text-[11px] text-emerald-600 bg-emerald-50 px-1.5 rounded-sm w-fit mt-0.5">
                              عميل
                            </span>
                          </div>
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="text-[11px] text-slate-500 bg-slate-100 px-1.5 rounded-sm w-fit mt-0.5">
                              مورد
                            </span>
                          </div>
                        ) : (
        ...
        <div className="fixed inset-0 bg-slate-800/50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 w-full max-w-md border border-slate-200 shadow-xl max-h-[90vh] overflow-y-auto">
            <h3 className="text-[17px] font-bold text-slate-800 mb-4">
              {editingTransaction
                ? t("edit_transaction_title", language)
                : t("add_transaction_title", language)}
            </h3>
            <form onSubmit={handleAddTransaction} className="space-y-4">
              <div>
                <label className="block text-[13px] font-semibold text-slate-600 mb-1">
                  {t("transaction_type", language)}
                </label>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <label className="flex items-center justify-center gap-2 p-3 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50 has-[:checked]:border-red-500 has-[:checked]:bg-red-50">
                    <input
                      ...
                      onChange={(e) => setTxType("expense" as TransactionType)}
                      className="hidden"
                    />
                    <ArrowDownRight className="w-4 h-4 text-red-600" />
                    <span className="text-sm font-medium text-slate-800">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-[13px] font-semibold text-slate-600 mb-1">
                    {t("payment_receipt_method", language)}
                  </label>
                  <select
              ...
              <div>
                ...
                <input
                  ...
                  name="description"
                  type="text"
                  placeholder={t("transaction_desc_placeholder", language)}
                  className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:outline-none text-sm"
                />
              </div>
              <div className="grid grid-cols-2 gap-4">
                {(txType === "income" ||
                  txType === "expense" ||
                  txType === "operating_expense") && (
                  <div>
                    <label className="block text-[13px] font-semibold text-slate-600 mb-1">
                      ارتباط بعميل (اختياري)
                    </label>
                    <select
                      name="customer_id"
                      ...
                      onChange={(e) => {
                        ...
                        if (e.target.value) setSelectedBooking("");
                      }}
                      disabled={!!selectedBooking}
                      className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:outline-none text-sm bg-white disabled:bg-slate-50 disabled:text-slate-400"
                    >
                ...
                {(txType === "expense" || txType === "operating_expense") && (
                  <div>
                    <label className="block text-[13px] font-semibold text-slate-600 mb-1">
                      {t("linked_supplier_optional", language)}
                    </label>
                    <select
                      name="supplier_id"
                ...
                {txType === "income" && (
                  <div>
                    <label className="block text-[13px] font-semibold text-slate-600 mb-1">
                      {t("linked_booking_optional", language)}
                    </label>
                    <select
                      name="booking_id"
                    ...
                    >
                      ...
                      {bookings.map((b) => {
                        const customer = customers.find(
                          (c) => c.id === b.customer_id,
                        );
                        return (
                          <option key={b.id} value={b.id}>
                            {customer?.name} -{" "}
                            {getCleanDescription(b.description)}
                          </option>
                        );
                      })}
                    </select>
        ...
        <div className="fixed inset-0 bg-slate-800/50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 w-full max-w-sm border border-slate-200 shadow-xl">
            ...
            <p className="text-slate-600 text-sm mb-6">
              {t("delete_transaction_confirm", language)}{" "}
              <strong>{transactionToDelete.description}</strong>?<br />
              <span className="text-[12px] text-slate-500">
                {t("delete_transaction_warning", language)}
              </span>
            </p>
            ...
            >
              <div>
                ...
                <input
                  ...
                  min="1"
                  step="1"
                  className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none text-sm font-bold"
                />
              </div>
              ...
              <div>
                <label className="block text-[13px] font-semibold text-slate-600 mb-1">
                  {t("transfer_desc_optional", language)}
                </label>
                <input
                  name="description"
                  type="text"
                  placeholder={t("transfer_desc_placeholder", language)}
