  ...
  return (
    <div className="flex flex-col gap-8 w-full max-w-5xl mx-auto" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      <div className="flex flex-col gap-2">
        <h1 className="text-2xl font-bold text-slate-800">{t('agency_settings', language)}</h1>
        ...
        <section className="flex flex-col gap-8">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6 flex flex-col h-fit">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-[17px] font-bold text-slate-800">{t('account_management', language)}</h2>
            </div>
                ...
                <div key={profile.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 border border-slate-100 rounded-lg hover:border-slate-200 transition-colors">
                  <div className="flex items-center gap-3 mb-3 sm:mb-0">
                    ...
                    <div>
                      ...
                      <p className="text-[12px] text-slate-500">{profile.role === 'admin' ? t('general_manager', language) : t('sales_agent', language)}</p>
                    </div>
                  </div>
                  {profile.id !== user?.id ? (
                    <select
                    ...
                    >
                      <option value="agent">{t('sales_agent', language)}</option>
                      <option value="admin">{t('general_manager', language)}</option>
                    </select>
                  ) : (
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6 flex flex-col h-fit">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="text-[17px] font-bold text-slate-800">{t('pos_staff', language)}</h2>
                <p className="text-[12px] text-slate-500 mt-1">{t('pos_staff_desc', language)}</p>
              </div>
            </div>
              ...
              <form onSubmit={handleAddLocalStaff} className="flex flex-col gap-3">
                <div className="flex flex-col sm:flex-row gap-2">
                  ...
                  >
                    ...
                    <option value="manager">{t('staff_manager', language)}</option>
                  </select>
                  <input 
                    type="password"
                    maxLength={4}
        ...
        <section className="bg-white rounded-xl border border-slate-200 p-6 flex flex-col h-fit">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-[17px] font-bold text-slate-800">{t('daily_report', language)}</h2>
            </div>
            <input 
              type="date" 
              value={reportDate}
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div className="p-4 rounded-lg bg-emerald-50 border border-emerald-100">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[13px] font-medium text-emerald-800">{t('cash_in', language)}</span>
              </div>
            ...
            <div className="p-4 rounded-lg bg-red-50 border border-red-100">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[13px] font-medium text-red-800">{t('cash_out', language)}</span>
              </div>
            ...
            <div className="col-span-2 p-4 rounded-lg bg-slate-50 border border-slate-200">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[13px] font-medium text-slate-600">{t('net_cash_flow', language)}</span>
                <span className={`text-[13px] font-bold ${dailyIncome - dailyExpense >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                  {dailyIncome - dailyExpense >= 0 ? t('surplus', language) : t('deficit', language)}
                </span>
              </div>
          ...
          <div className="border-t border-slate-100 pt-5">
            <h3 className="text-[14px] font-bold text-slate-800 mb-3">{t('daily_bookings_summary', language)}</h3>
            <ul className="space-y-3">
              <li className="flex justify-between items-center text-[13px]">
                <span className="text-slate-500">{t('created_bookings', language)}</span>
                <span className="font-bold text-slate-800">{dailyBookings.length} {t('bookings_count', language)}</span>
              </li>
              <li className="flex justify-between items-center text-[13px]">
                <span className="text-slate-500">{t('today_sales_volume', language)}</span>
              ...
              <li className="flex justify-between items-center text-[13px] pt-3 border-t border-slate-50">
                <span className="text-slate-500 font-semibold">{t('today_expected_profits', language)}</span>
