  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!authData || !authData.user)
        throw new Error("فشل إنشاء الحساب أو حدث خطأ في الشبكة");
      ...
      if (agencyError) {
        ...
        throw new Error(
          "تم إنشاء الحساب لكن تم حجب الوكالة. (خطأ في إدراج الوكالة)",
        );
      }
      ...
      if (err.message === "TIMEOUT") {
        ...
        if (
          ...
          errorMessage =
            "رقم الهاتف هذا مسجل مسبقاً في النظام. يرجى تسجيل الدخول.";
        } else if (err.message.includes("Password should be at least")) {
          errorMessage = "يجب أن تتكون كلمة المرور من 6 أحرف على الأقل.";
        } else if (err.message.includes("Email not confirmed")) {
          errorMessage =
            "يرجى تفعيل الحساب أو التأكد من إعدادات Supabase (تعطيل Confirm Email)";
        }
      }
      ...
      <div className="bg-white w-full max-w-md rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
        <div className="bg-slate-800 p-6 text-center">
          ...
          <p className="text-slate-400 text-xs">
            نظام بوصلة لإدارة وكالات السفر
          </p>
        </div>
