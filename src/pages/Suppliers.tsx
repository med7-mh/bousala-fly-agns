          ...
          <table className="w-full text-right border-collapse">
            <thead>
              <tr>
                ...
                <th className="py-3 px-2 border-b-2 border-slate-100 text-slate-400 text-[13px] font-semibold whitespace-nowrap">
                  {t("total_claims", language)}
                </th>
                <th className="py-3 px-2 border-b-2 border-slate-100 text-slate-400 text-[13px] font-semibold whitespace-nowrap">
                  {t("paid_to_them", language)}
                </th>
                <th className="py-3 px-2 border-b-2 border-slate-100 text-slate-400 text-[13px] font-semibold whitespace-nowrap">
                  {t("remaining_balance", language)}
                </th>
            ...
            <tbody>
              {filteredSuppliers.map((supplier) => {
                ...
                const totalPaid = supplierTransactions.reduce((sum, t) => {
                    ...
                    <td className="py-3.5 px-2 border-b border-slate-50 text-[14px] font-bold whitespace-nowrap">
                      {debt > 0 ? (
                        <span className="text-red-600">
                          {formatCurrency(debt)} (لهم)
                        </span>
                      ) : debt < 0 ? (
                        <span className="text-emerald-600">
                          {formatCurrency(Math.abs(debt))} (لنا)
                        </span>
                      ) : (
        ...
        <div className="fixed inset-0 bg-slate-800/50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 w-full max-w-md border border-slate-200 shadow-xl max-h-[90vh] overflow-y-auto">
            <h3 className="text-[17px] font-bold text-slate-800 mb-4">
              {editingSupplier
                ? t("edit_supplier_title", language)
                : t("new_supplier_title", language)}
            </h3>
        ...
        <div className="fixed inset-0 bg-slate-800/50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 w-full max-w-sm border border-slate-200 shadow-xl">
            ...
            <p className="text-slate-600 text-sm mb-6">
              ...
              <strong>{supplierToDelete.name}</strong>؟<br />
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-[19px] font-bold text-slate-800">
                {t("supplier_statement", language)}: {statementSupplier.name}
              </h3>
              <div
                  ...
                  {(() => {
                    ...
                    const sp = transactions
                      ...
                      .map((t) => ({
                        ...
                        desc:
                          t.type === "expense"
                            ? `دفعة للمورد: ${t.description} (${t.payment_method || "نقدي"})`
                            : `مسترد من المورد: ${t.description} (${t.payment_method || "نقدي"})`,
                        debit: t.type === "income" ? t.amount : 0, // When they refund us, it increases our debt temporarily in this ledger perspective
                        credit: t.type === "expense" ? t.amount : 0, // Payment to them
                      }));

                    const ledger = [...sb, ...sp].sort(
                      (a, b) => a.date - b.date,
                    );
